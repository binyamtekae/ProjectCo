...
async function getFacts() {
 ...
 query = query.eq("category", currentCategory)
  ...
  .order('votesFalse',{ascending: false})
  .limit(1000);
...
return ( 
  <>
  <Header showForm={showForm} setShowForm=
  {setShowForm} />
...
async function handleSubmit(e){
  ...
  // 2. check if data is valid. if so creat a new fact
  if(text && isValidHttpUrl(source) && category && textLength <= 200)
  {
  //3. Upload fact to supabase and receive the new fact update
   setIsUploading(true);
  ...
  disabled= {isUploading}>Post</button></form>
}
...
return ( 
   <li  className="fact">
  <p>
        {isDisputed ? <span className='disputed'>[⛔️ DISPUTED]
        </span> : null}
        {fact.text}
